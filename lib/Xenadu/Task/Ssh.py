...
def ssh_deploy_server_keys(dummy):
    ...
    X = Language()
    X.add("/etc/ssh/ssh_host_dsa_key", "_ssh/ssh_host_dsa_key", X.root_600)
    X.add("/etc/ssh/ssh_host_dsa_key.pub", "_ssh/ssh_host_dsa_key.pub")
    X.add("/etc/ssh/ssh_host_rsa_key", "_ssh/ssh_host_rsa_key", X.root_600)
    X.add("/etc/ssh/ssh_host_rsa_key.pub", "_ssh/ssh_host_rsa_key.pub")
    mapping = X.get_hash()
...
def ssh_dom0(command):
    ...
    output = subprocess.Popen(["/usr/bin/ssh",
        "%(user)s@%(address)s" % Xenadu.Env["Config"]["dom0"]["ssh"],
        command],
        stdout=subprocess.PIPE).communicate()[0]
...
def register():
	...
	Xenadu.Env["Registry"].register_task(name="ssh.keys.deploy", args=0, help="copy ssh keys over", function=ssh_deploy_server_keys)
	Xenadu.Env["Registry"].register_task(name="ssh.keys.import", args=0, help="grab ssh keys from remote server", function=ssh_import_server_keys)
